 ...
 * freedesktop field codes are stripped.
 */
...
export function resolveZcodeBin(): string | null {
  ...
  if (process.platform === "linux") {
    ...
    // `.cjs`-direct branch then runs it without mounting.
...
export function defaultZcodeCjsPaths(): string[] {
  ...
  // The official `.deb` install lays a loose
...
export type DetectedAgent = {
   ...
   * keyed personal provider); false with {@link notReadyReason} when the
   * user still needs to log in or add a provider in ZCode. The settings
   * card renders the amber "已安装 · 未就绪" state from these. Absent for
   * every other agent (no readiness notion).
   */
  ready?: boolean;
...
export function detectAgents(): DetectedAgent[] {
  return AGENTS.map((a): DetectedAgent => {
    ...
    // usable keyed provider) surfaces the amber card state while keeping the
    // static floor.
