...
// for a spawn/stdio unit test and matches how cli runs the same code path.
...
const { mockSpawn, existsSyncDelegate, mockMkdtempSync, mockWriteFileSync, mockRmSync, mockPrepareBinding, bindingOkResult } = vi.hoisted(() => {
  return {
    ...
    // argv-attach (zcode) prompt-delivery path: the invoke layer mkdtemps a
    // temp dir, writes prompt.md into it, and rmSyncs the dir on every exit
...
describe("invokeAgent zcode CLI one-shot (argv-attach)", () => {
  ...
  it("spawns `node <cjs> -p <guide> --attach <tmp> --output-format stream-json --mode yolo` and reports start.argv", async () => {
    ...
    // elsewhere: argv elements verbatim on every host, so spaced install
    // paths survive with no quoting games.
  ...
  it("resets the silence clock on every parsed event", async () => {
    ...
    // would have fired at t=180s. The clock restarted, so it is still armed.
    vi.advanceTimersByTime(100_000);
    await flushMicrotasks();
  ...
  it("does not arm the watchdog for non-zcode agents", async () => {
    ...
    await flushMicrotasks();
    // Never armed for a sibling agent, so a full silence window + close
    // produces no watchdog error and the close path still reaches done
    // (historical behavior intact).
    expect(vi.getTimerCount()).toBe(0);
