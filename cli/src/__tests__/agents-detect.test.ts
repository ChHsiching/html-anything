...
// call. Mock only that; everything else stays the real implementation
// (keep the module's exports real so this mock keeps working).
...
describe("detectAgents", () => {
  ...
  describe("resolveZcodeNodeBin", () => {
    ...
    it("Linux: falls back to the AppImage mount", () => {
      // On Linux the AppImage doubles as the Electron driver (run under
      // ELECTRON_RUN_AS_NODE=1 it acts as node).
      stubPlatform("linux");
  ...
  describe("ZCode agent registration", () => {
    ...
    // rather than the literal "node". On a clean host where only the ZCode
    ...
    // single reason, and the registry holds nothing so the picker keeps the
    // static floor.
...
// only host without a system node can still drive the .cjs with no mount needed.
